...
# Build Pure Nexus with one easy script. You will need to have the repo
...
# SOURCE_DIR: The directory that holds your PN repos. This is automatically
#             two folders above the scripts location as it is located in
#             vendor/pure. Since the script will be symlinked by the manifest,
#             we need to use readlink to get where it actually is.
...
# DEST_DIR: The directory that will hold your completed PN zip files for ease of
...
function get_devices {
    ...
    PRODUCTS=$( ls "${SOURCE_DIR}"/vendor/pure/products/* )

    for TEMP in ${PRODUCTS}; do
        DEVICE_LIST+="$( basename "${TEMP}" | sed s/.mk// ) "
    done
...
function help_menu() {
    ...
    echo -e "${BOLD}OVERVIEW:${RST} Build Pure Nexus from source!\n"
    ...
    echo -e "               Add shallow as a parameter for a shallow sync"
    echo -e "               Default is no sync\n"
    echo -e "   variant:    Takes a value of user, userdebug, or eng"
    echo -e "               This is only needed with advanced users\n"
...
while [[ $# -ge 1 ]]; do
    ...
    case "${1}" in
        "type")
            shift
            if [[ $# -ge 1 ]]; then
                export PURE_BUILD_TYPE=${1}
            else
                reportError "A custom type was requested but not specified!"
                exit
            fi ;;
        "sync")
            SYNC=true ;;
        ...
        "variant")
            ...
            if [[ $# -ge 1 ]]; then
                ...
                reportError "Variant was requested but not specified!"
                help_menu
            fi ;;
        "noclean")
            CLEAN=false ;;
...
if [[ ${SYNC} = true ]]; then
    ...
    if [[ ${SHALLOW} = true ]]; then
        SYNCFLAGS="-c --no-clone-bundle --no-tags --optimized-fetch --prune"
    fi
    repo sync --force-sync -j$( nproc --all ) ${SYNCFLAGS}
fi
...
if [[ $( echo ${VARIANT} | grep -v -e user -e userdebug -e eng ) ]]; then
    reportWarning "A variant is given but it's invalid; falling back to default"
    unset VARIANT
fi
